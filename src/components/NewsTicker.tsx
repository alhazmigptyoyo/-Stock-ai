...
import { Radio, Flame, ExternalLink, Clock, Pause, Play, TrendingUp, AlertCircle } from 'lucide-react';
...
export const NewsTicker: React.FC<NewsTickerProps> = ({ onSelectSymbol }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % DEFAULT_NEWS.length);
    }, 6000);
    return () => clearInterval(interval);
  }, [isPlaying]);

  const currentItem = DEFAULT_NEWS[activeIndex];

  return (
    <div className="bg-slate-900/90 border-b border-slate-800/80 backdrop-blur-md py-1.5 px-3 sm:px-6 text-xs text-slate-300 flex items-center justify-between gap-3 overflow-hidden select-none">
      ...
      <div className="flex-1 min-w-0 flex items-center gap-2 justify-center sm:justify-start">
        <span className="text-[10px] px-1.5 py-0.5 rounded font-bold bg-amber-950/80 border border-amber-700/60 text-amber-300 flex-shrink-0 hidden sm:inline-block">
          {currentItem.category === 'SAUDI' ? 'السوق السعودي' : currentItem.category === 'US' ? 'السوق الأمريكي' : 'أسواق عالمية'}
        </span>

        <p className="truncate text-slate-200 font-medium text-xs sm:text-[13px] hover:text-amber-300 transition-colors cursor-pointer" onClick={() => currentItem.link && window.open(currentItem.link, '_blank')}>
          {currentItem.title}
        </p>

        <span className="text-[10px] text-slate-500 flex items-center gap-1 flex-shrink-0 hidden md:flex">
        ...
        {currentItem.relatedSymbol && onSelectSymbol && (
          <button
            ...
            className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-emerald-950 border border-emerald-700 text-emerald-300 hover:bg-emerald-900 transition-colors flex-shrink-0 hidden lg:inline-block"
          >
      ...
      <div className="flex items-center gap-1.5 flex-shrink-0">
        <div className="flex items-center gap-1 text-[10px] text-slate-400 font-mono">
          ...
          <span>/</span>
          <span>{DEFAULT_NEWS.length}</span>
        </div>

        <button
          onClick={() => setActiveIndex((prev) => (prev - 1 + DEFAULT_NEWS.length) % DEFAULT_NEWS.length)}
          className="px-1.5 py-0.5 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors text-[10px] font-bold"
        >
          ‹
        </button>
        <button
          onClick={() => setActiveIndex((prev) => (prev + 1) % DEFAULT_NEWS.length)}
          className="px-1.5 py-0.5 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 transition-colors text-[10px] font-bold"
        >
          ›
        </button>
